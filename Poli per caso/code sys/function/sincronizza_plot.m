function sincronizza_plot(T, u, nome_test, soglia, Ts, Tend, x0, theta)
    ...
    [t, x, y] = FFD(Ts, Tend-1, x0, u, theta);
    indice_scalino2 = find(y(2,:) > soglia, 1, 'first');
    
    t_exp = t(indice_scalino:indice_scalino+200) - t(indice_scalino);
    t_mod = t(indice_scalino2:indice_scalino2+200) - t(indice_scalino2);
    ...
    figure();
    hold on;
    plot(t_exp, T(2, indice_scalino:indice_scalino+200), 'b-', 'LineWidth', 1);
    plot(t_mod, y(2, indice_scalino2:indice_scalino2+200), 'r-', 'LineWidth', 1);
    grid on;
