...
typedef RT_MODEL_feedback_2019_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * feedback_2019(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void feedback_2019_initialize();
   extern void feedback_2019_terminate();
 */
...
static void rtExtModeSingleTaskUpload(RT_MODEL * S)
{
  {                                    /* Sample time: [0.0s, 0.0s] */
    rtExtModeUpload(0, feedback_2019_M->Timing.t[0]);
  }

  if (rtmIsMajorTimeStep(feedback_2019_M)) {/* Sample time: [0.002s, 0.0s] */
    rtExtModeUpload(1, (((feedback_2019_M->Timing.clockTick1+
                          feedback_2019_M->Timing.clockTickH1* 4294967296.0)) *
                        0.002));
  }
}
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(feedback_2019_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://feedback_2019:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: feedback_2019 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
